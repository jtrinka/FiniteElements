...
function [Wall] = AdvectionDiffusionFiniteElementsConstant(xy,nodes,neumann,Dirich,savexsizemin,saveysizemin,savexsizemax,saveysizemax,f,v,D,T,dt,wall)
...
%   if n==2 || n==1000 || n==2000 || n==3000 || n==4000 || n==5000 || n==5600
% frame = getframe(1);
%   im = frame2im(frame);
%   [Matrix,map] = rgb2ind(im,256);
%   Animationpic = sprintf('mathconst_img_%d.png', n) ;
...
%  EndVec=U(:,end); 
 ...
 figure
plot(Wall)
title('Increase in Concentration of Contaminant')
xlabel('Time-Step (n)')
ylabel('Total Concentration of Contaminant(kg per square meter)')
end
